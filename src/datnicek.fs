...
let globalVisObj =
  { Name = "vis"
    Members = [
      ...
      { Name = "scatter"; Arguments = [ "data", "table" ]; Type = fun () -> chartObj
        Invoke = fun _ -> failwith "not implemented" }
      { Name = "column"; Arguments = [ "data", "table" ]; Type = fun () -> chartObj
        Invoke = fun _ -> failwith "not implemented" }
...
type State =
  {
    ...
    // Text editing
    SelectedParagraph : Map<string, string option>
  }
...
type Event =
  ...
  // Text editing
  | EditParagraph of call:string * id:string option
...
let getBindings doc =
  ...
  | Record(_, cells) ->
      cells |> OrdList.toList |> List.collect (fun (ck, cv) ->
        ...
        | Record("cell-grid", Lookup(Find "name" (Primitive(String nm)))) when nm <> "" -> 
...
let markdownPrefixes = 
  [ yield! [ "#","h1",None; "##","h2",None; "###","h3",None; "*","li",Some("ul"); ">","blockquote",None ] 
    for i in 0 .. 9 -> $"{i}.","li",Some("ol") ]

let getMarkdownLineInfo (s:string) =
  markdownPrefixes 
  |> List.tryPick (fun (prefix,tag,group) -> 
    if s.StartsWith(prefix + " ") then Some(tag, group, s.Substring(prefix.Length+1)) else None)
  |> Option.defaultValue ("p", None, s)

let (|DelimitedWith|_|) cos ccs0 cs = 
  let rec finish acc cs ccs =
    match cs, ccs with 
    | cs, [] -> Some(List.rev acc, cs)
    | c::cs, cc::ccs when c = cc -> finish acc cs ccs
    | c::cs, _ -> finish (c::acc) cs ccs0
    | [], _ -> None
  let rec start cs cos =
    match cs, cos with 
    | cs, [] -> finish [] cs ccs0
    | c::cs, co::cos when c = co -> start cs cos
    | _ -> None
  start cs cos

let formatMarkdownSpan str = 
  let rec parse chars = 
    let rec loop acc chars = 
      match chars with
      | DelimitedWith ['_';'_'] ['_';'_'] (body, chars) 
      | DelimitedWith ['*';'*'] ['*';'*'] (body, chars) ->
          loop ((h?strong [] (parse body))::acc) chars
      | DelimitedWith ['_'] ['_'] (body, chars) 
      | DelimitedWith ['*'] ['*'] (body, chars) ->
          loop ((h?em [] (parse body))::acc) chars
      | DelimitedWith ['['] [']'] (body, DelimitedWith ['('] [')'] (url, chars)) ->
          loop ((h?a ["href" => System.String(Array.ofList url) ] (parse body))::acc) chars
      | DelimitedWith ['`'] ['`'] (body, chars) ->
          loop ((h?code [] (parse body))::acc) chars
      | c::cs -> loop ((text (string c))::acc) cs
      | [] -> List.rev acc
    loop [] chars      
  parse (List.ofSeq str)

let renderTextCell trigger state cell =
    Log.logOp "apprender" $"renderTextCell {cell.Id}" <| fun () ->
  let content = 
    match select [Field cell.Id] state.Document with 
    | [ Record(_, content) ] -> 
        [ for k, nd in content do
            match nd with PS s -> k, s | _ -> failwith "renderTextCell: Line not string" ]
    | _ -> failwith "renderTextCell: cell not found"
  let content = if content.Length = 0 then [ "ln-00000000", "" ] else content
  let beforeAfter = dict [ for b,v,a in List.withBeforeAfter (List.map fst content) -> v, (b, a) ]

  let saveLine pid line = 
    let b, a = beforeAfter.[pid]
    let ed = RecordAdd([Field cell.Id], pid, b, a, ps line)
    trigger(ApplyEdits("edit text", [ ed ]))

  let deleteLine pid line = 
    let b, a = beforeAfter.[pid]
    let ed = RecordDelete(UpdateReferences, [Field cell.Id], pid)
    trigger(ApplyEdits("edit text", [ ed ]))
    trigger(EditParagraph(cell.Id, a |> Option.orElse b))

  let saveLineNlAfter pid line = 
    let b, a = beforeAfter.[pid]
    let newId = uniqueId "ln" 
    let eds = [
      RecordAdd([Field cell.Id], newId, Some pid, a, ps "") 
      RecordAdd([Field cell.Id], pid, b, Some newId, ps line) ]
    trigger(ApplyEdits("edit text", eds))
    trigger(EditParagraph(cell.Id, Some newId))

  let saveLineNlBefore pid line = 
    let b, a = beforeAfter.[pid]
    let newId = uniqueId "ln" 
    printfn $"b,a = {(b,a)}, new = {newId}"
    let eds = [
      RecordAdd([Field cell.Id], newId, b, Some pid, ps "") 
      RecordAdd([Field cell.Id], pid, Some newId, a, ps line) 
      ]
    trigger(ApplyEdits("edit text", eds))
    trigger(EditParagraph(cell.Id, Some newId))

  let renderLine (pid, tag, content) =
    h?(tag) [ "click" =!> fun _ _ -> trigger(EditParagraph(cell.Id, Some pid)) ] (formatMarkdownSpan content)

  let renderSelLine (pid, line, tag, content) =
    h?(tag) [
      "class" => "selected" 
      "keydown" =!> fun el evt ->
        let ke = unbox<KeyboardEvent> evt
        let line = (unbox<HTMLElement> el).innerText
        if ke.key = "Delete" && ke.shiftKey then
          deleteLine pid line
          evt.preventDefault()
        if ke.key = "Enter" then
          if ke.ctrlKey then saveLineNlBefore pid line
          else saveLineNlAfter pid line
          evt.preventDefault()
        if ke.key = "Escape" then
          saveLine pid line
          trigger(EditParagraph(cell.Id, None))
          evt.preventDefault()
        if ke.key = "ArrowDown" then 
          evt.preventDefault()
          saveLine pid line
          snd beforeAfter.[pid] |> Option.iter (fun pid -> trigger(EditParagraph(cell.Id, Some pid)))
        if ke.key = "ArrowUp" then 
          evt.preventDefault()
          saveLine pid line
          fst beforeAfter.[pid] |> Option.iter (fun pid -> trigger(EditParagraph(cell.Id, Some pid)))
      "contenteditable" => "true"
      "click" =!> fun _ _ -> trigger(EditParagraph(cell.Id, Some pid))
    ] [ text line ]
    |> h.onload (fun el -> el.focus())

  let selectedPid = 
    match state.SelectedParagraph.TryFind cell.Id with 
    | Some pid when state.SelectedCell = Some cell.Id -> pid 
    | _ when state.SelectedCell = Some cell.Id -> Some(fst (List.head content))
    | _ -> None
  Log.log "markdown" $"Selected: {selectedPid}, Content: {content}" 

  let lines = content |> List.map (fun (pid, line) -> 
    let tag, group, content = getMarkdownLineInfo line
    let selected = Some pid = selectedPid
    if not selected then group, renderLine (pid, tag, content)          
    else group, renderSelLine (pid, line, tag, content) )
  
  let grouped = lines |> List.groupAdjacent fst
  h?div [ "class" => "text-body" ] [
    for gtag, group in grouped do
      let group = List.map snd group
      match gtag with 
      | None -> yield! group          
      | Some tag -> 
          yield h?(tag) [] group 
          // Generate empty invisible elements to avoid confusing virtualdom
          for i in 1 .. group.Length-1 -> h?div [] [] ]  
...
// --------------------------------------------------------------------------------------
// Serializing and deserializing events (for the purpose of essay replays)
// --------------------------------------------------------------------------------------
...
module EventLogger =
  ...
  // Primitive option helpers

  let strOptToJson = function Some (s:string) -> box s | None -> jnull
  let strOptFromJson o = if isNull o then None else Some(unbox<string> o)

  let strListToJson (xs:string list) = box [| for s in xs -> box s |]
  let strListFromJson o = [ for s in unbox<obj[]> o -> unbox<string> s ]
  let strListOptToJson = function Some xs -> strListToJson xs | None -> jnull
  let strListOptFromJson o = if isNull o then None else Some(strListFromJson o)

  // Selectors (a list of selectors) and optional selectors

  let selsToJson (sels:Selectors) = box [| for s in sels -> selToJson s |]
  let selsFromJson o = [ for s in unbox<obj[]> o -> selFromJson s ]
  let selsOptToJson = function Some sels -> selsToJson sels | None -> jnull
  let selsOptFromJson o = if isNull o then None else Some(selsFromJson o)

  // Edits (serialized as nodes, via Represent, like in Webnicek)

  let editKindToJson (ek:EditKind) =
    nodeToJson (Represent.represent None { Kind = ek; Dependencies = []; GroupLabel = "" })
  let editKindFromJson o =
    (Represent.unrepresent (nodeFromJson o) |> List.head |> fst).Kind
  let editKindsToJson (eks:EditKind list) = box [| for ek in eks -> editKindToJson ek |]
  let editKindsFromJson o = [ for o in unbox<obj[]> o -> editKindFromJson o ]

  // Grid editing types

  let gridLocationToJson (l:GridLocation) =
    jsObj [ "cellId", box l.CellId; "rowIndex", strOptToJson l.RowIndex; "field", box l.Field ]
  let gridLocationFromJson o : GridLocation =
    { CellId = o?cellId; RowIndex = strOptFromJson o?rowIndex; Field = o?field }
  let gridLocationOptToJson = function Some l -> gridLocationToJson l | None -> jnull
  let gridLocationOptFromJson o = if isNull o then None else Some(gridLocationFromJson o)

  let editExpansionToJson (e:EditExpansion) =
    jsObj [ "target", selsToJson e.Target; "condition", selsToJson e.Condition; "prefix", selsToJson e.Prefix ]
  let editExpansionFromJson o : EditExpansion =
    { Target = selsFromJson o?target; Condition = selsFromJson o?condition; Prefix = selsFromJson o?prefix }
  let editExpansionOptToJson = function Some e -> editExpansionToJson e | None -> jnull
  let editExpansionOptFromJson o = if isNull o then None else Some(editExpansionFromJson o)

  let expandedEditToJson (e:ExpandedEdit) =
    jsObj [ "expansion", editExpansionOptToJson e.Expansion; "edit", editKindToJson e.Edit ]
  let expandedEditFromJson o : ExpandedEdit =
    { Expansion = editExpansionOptFromJson o?expansion; Edit = editKindFromJson o?edit }

  let gridEditToJson (g:GridEdit) =
    jsObj [
      "title", box g.Title; "icon", box g.Icon; "editType", box g.EditType
      "description", box g.Description
      "edits", box [| for e in g.Edits -> expandedEditToJson e |]
      "code", nodeToJson g.Code ]
  ...
  let serializeEvent e : obj option =
    match e with
    | ToggleViewSource -> jsObj [ "kind", box "toggleViewSource" ] |> Some
    | ToggleShowHistory -> jsObj [ "kind", box "toggleShowHistory" ] |> Some
    | UndoLastEdit -> jsObj [ "kind", box "undoLastEdit" ] |> Some
    | ToggleMenu path -> jsObj [ "kind", box "toggleMenu"; "path", strListOptToJson path ] |> Some
    | SelectCell cell -> jsObj [ "kind", box "selectCell"; "cell", strOptToJson cell ] |> Some
    | ApplyEdits(lbl, eds) -> jsObj [ "kind", box "applyEdits"; "groupLabel", box lbl; "edits", editKindsToJson eds ] |> Some
  ...
  let deserializeEvent (o:obj) =
    match o?kind with
    | "toggleViewSource" -> ToggleViewSource
    | "toggleShowHistory" -> ToggleShowHistory
    | "undoLastEdit" -> UndoLastEdit
    | "toggleMenu" -> ToggleMenu(strListOptFromJson o?path)
    | "selectCell" -> SelectCell(strOptFromJson o?cell)
    | "applyEdits" -> ApplyEdits(o?groupLabel, editKindsFromJson o?edits)
  ...
  let deserializeEvents (o:obj) =
    unbox<obj[]> o |> Array.map deserializeEvent
...
module Loader =
  ...
  //
  //let json = """[{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[]}],["field","cell-76e89cf2"],["node",{"kind":"record","tag":"cell-code","nodes":[]}]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"]]}],["field","unnamed-af5e506"],["node",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","data"]}]]},{"kind":"record","tag":"x-edit-wraprec","nodes":[["tag","inst"],["fld","x-formula"],["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"],["1","unnamed-af5e506"]]}],["refs","keep"]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"],["1","unnamed-af5e506"]]}],["field","op"],["node",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","data","load"]}]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"],["1","unnamed-af5e506"]]}],["field","file"],["node",""]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"],["1","unnamed-af5e506"]]}],["field","file"],["node","eurostat/sf_aviaca_eukil.tsv"]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"]]}],["field","unnamed-ef1e48da"],["node",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","data"]}],["pred","unnamed-af5e506"]]},{"kind":"record","tag":"x-edit-wraprec","nodes":[["tag","inst"],["fld","x-formula"],["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"],["1","unnamed-ef1e48da"]]}],["refs","keep"]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"],["1","unnamed-ef1e48da"]]}],["field","op"],["node",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","data","load"]}]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"],["1","unnamed-ef1e48da"]]}],["field","file"],["node",""]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"],["1","unnamed-ef1e48da"]]}],["field","file"],["node","eurostat/sf_railvi_totalkil.tsv"]]},{"kind":"record","tag":"x-edit-renamefld","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"]]}],["old","unnamed-af5e506"],["new","aviaRaw"],["refs","update"]]},{"kind":"record","tag":"x-edit-renamefld","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-76e89cf2"]]}],["old","unnamed-ef1e48da"],["new","railRaw"],["refs","update"]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[]}],["field","cell-6cdc1d54"],["node",{"kind":"record","tag":"cell-grid","nodes":[]}],["pred","cell-76e89cf2"]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-6cdc1d54"]]}],["field","target"],["node",{"kind":"reference","refkind":"absolute","selectors":["cell-76e89cf2","aviaRaw"]}]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-6cdc1d54"]]}],["field","edits"],["node",{"kind":"list","tag":"edits","nodes":[]}]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-6cdc1d54"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Split column by delimiter"],["icon","fa-scissors"],["description","Split column `freq,unit,victim,c_regis,geo\\TIME_PERIOD' by delimiter `,'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","split"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","freq,unit,victim,c_regis,geo\\TIME_PERIOD"],["by",","]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","unit"],["node",""],["pred","freq,unit,victim,c_regis,geo\\TIME_PERIOD"]]}],["1",{"kind":"record","tag":"x-edit-copy","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","unit"]]}],["source",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq,unit,victim,c_regis,geo\\TIME_PERIOD"]]}],["refs","update"]]}],["2",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","unit"]]}],["op","take-by"],["arg",",/1"]]}],["3",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","victim"],["node",""],["pred","unit"]]}],["4",{"kind":"record","tag":"x-edit-copy","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","victim"]]}],["source",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq,unit,victim,c_regis,geo\\TIME_PERIOD"]]}],["refs","update"]]}],["5",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","victim"]]}],["op","take-by"],["arg",",/2"]]}],["6",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","c_regis"],["node",""],["pred","victim"]]}],["7",{"kind":"record","tag":"x-edit-copy","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","c_regis"]]}],["source",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq,unit,victim,c_regis,geo\\TIME_PERIOD"]]}],["refs","update"]]}],["8",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","c_regis"]]}],["op","take-by"],["arg",",/3"]]}],["9",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","geo\\TIME_PERIOD"],["node",""],["pred","c_regis"],["succ","2006"]]}],["10",{"kind":"record","tag":"x-edit-copy","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","geo\\TIME_PERIOD"]]}],["source",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq,unit,victim,c_regis,geo\\TIME_PERIOD"]]}],["refs","update"]]}],["11",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","geo\\TIME_PERIOD"]]}],["op","take-by"],["arg",",/4"]]}],["12",{"kind":"record","tag":"x-edit-renamefld","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["old","freq,unit,victim,c_regis,geo\\TIME_PERIOD"],["new","freq"],["refs","update"]]}],["13",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq"]]}],["op","take-by"],["arg",",/0"]]}]]}]]}],["index","0"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-6cdc1d54"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Drop columns"],["icon","fa-rectangle-xmark"],["description","Drop column `freq'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","drop"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","freq"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","freq"]]}]]}]]}],["index","1"],["pred","0"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-6cdc1d54"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Drop columns"],["icon","fa-rectangle-xmark"],["description","Drop column `unit'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","drop"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","unit"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","unit"]]}]]}]]}],["index","2"],["pred","1"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-6cdc1d54"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Drop columns"],["icon","fa-rectangle-xmark"],["description","Drop column `victim'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","drop"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","victim"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","victim"]]}]]}]]}],["index","3"],["pred","2"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-6cdc1d54"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Keep rows by value"],["icon","fa-circle-check"],["description","Keep only rows with value `EU27_2020' in column `c_regis'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","filter"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","c_regis"],["cond","nequals"],["value","EU27_2020"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","$cond"],["node",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$builtins","nequals"]}],["left","EU27_2020"],["right",{"kind":"reference","refkind":"relative","selectors":["..","..","c_regis"]}]]}]]}],["1",{"kind":"record","tag":"x-expandable-edit","nodes":[["condition",{"kind":"reference","refkind":"relative","selectors":["$cond"]}],["prefix",{"kind":"reference","refkind":"absolute","selectors":["#0"]}],["target",{"kind":"reference","refkind":"absolute","selectors":["*"]}],["edit",{"kind":"record","tag":"x-edit-listdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[]}],["index","0"]]}]]}],["2",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","$cond"]]}]]}]]}],["index","4"],["pred","3"]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[]}],["field","cell-4b11c72b"],["node",{"kind":"record","tag":"cell-grid","nodes":[]}],["pred","cell-6cdc1d54"]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"]]}],["field","target"],["node",{"kind":"reference","refkind":"absolute","selectors":["cell-76e89cf2","railRaw"]}]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"]]}],["field","edits"],["node",{"kind":"list","tag":"edits","nodes":[]}]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Split column by delimiter"],["icon","fa-scissors"],["description","Split column `freq,unit,accident,victim,pers_cat,geo\\TIME_PERIOD' by delimiter `,'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","split"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","freq,unit,accident,victim,pers_cat,geo\\TIME_PERIOD"],["by",","]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","unit"],["node",""],["pred","freq,unit,accident,victim,pers_cat,geo\\TIME_PERIOD"]]}],["1",{"kind":"record","tag":"x-edit-copy","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","unit"]]}],["source",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq,unit,accident,victim,pers_cat,geo\\TIME_PERIOD"]]}],["refs","update"]]}],["2",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","unit"]]}],["op","take-by"],["arg",",/1"]]}],["3",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","accident"],["node",""],["pred","unit"]]}],["4",{"kind":"record","tag":"x-edit-copy","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","accident"]]}],["source",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq,unit,accident,victim,pers_cat,geo\\TIME_PERIOD"]]}],["refs","update"]]}],["5",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","accident"]]}],["op","take-by"],["arg",",/2"]]}],["6",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","victim"],["node",""],["pred","accident"]]}],["7",{"kind":"record","tag":"x-edit-copy","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","victim"]]}],["source",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq,unit,accident,victim,pers_cat,geo\\TIME_PERIOD"]]}],["refs","update"]]}],["8",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","victim"]]}],["op","take-by"],["arg",",/3"]]}],["9",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","pers_cat"],["node",""],["pred","victim"]]}],["10",{"kind":"record","tag":"x-edit-copy","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","pers_cat"]]}],["source",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq,unit,accident,victim,pers_cat,geo\\TIME_PERIOD"]]}],["refs","update"]]}],["11",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","pers_cat"]]}],["op","take-by"],["arg",",/4"]]}],["12",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","geo\\TIME_PERIOD"],["node",""],["pred","pers_cat"],["succ","2006"]]}],["13",{"kind":"record","tag":"x-edit-copy","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","geo\\TIME_PERIOD"]]}],["source",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq,unit,accident,victim,pers_cat,geo\\TIME_PERIOD"]]}],["refs","update"]]}],["14",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","geo\\TIME_PERIOD"]]}],["op","take-by"],["arg",",/5"]]}],["15",{"kind":"record","tag":"x-edit-renamefld","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["old","freq,unit,accident,victim,pers_cat,geo\\TIME_PERIOD"],["new","freq"],["refs","update"]]}],["16",{"kind":"record","tag":"x-edit-primitive","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"],["1","freq"]]}],["op","take-by"],["arg",",/0"]]}]]}]]}],["index","0"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Rename columns"],["icon","fa-i-cursor"],["description","Rename column `geo\\TIME_PERIOD' to `geo'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","rename"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["old","geo\\TIME_PERIOD"],["new","geo"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-renamefld","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["old","geo\\TIME_PERIOD"],["new","geo"],["refs","update"]]}]]}]]}],["index","1"],["pred","0"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Drop columns"],["icon","fa-rectangle-xmark"],["description","Drop column `freq'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","drop"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","freq"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","freq"]]}]]}]]}],["index","2"],["pred","1"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Drop columns"],["icon","fa-rectangle-xmark"],["description","Drop column `unit'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","drop"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","unit"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","unit"]]}]]}]]}],["index","3"],["pred","2"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Drop columns"],["icon","fa-rectangle-xmark"],["description","Drop column `accident'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","drop"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","accident"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","accident"]]}]]}]]}],["index","4"],["pred","3"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Drop columns"],["icon","fa-rectangle-xmark"],["description","Drop column `victim'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","drop"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","victim"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","victim"]]}]]}]]}],["index","5"],["pred","4"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Delete rows by value"],["icon","fa-circle-xmark"],["description","Delete rows with value `LV_USR' in column `pers_cat'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","filter"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","pers_cat"],["cond","equals"],["value","LV_USR"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","$cond"],["node",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$builtins","equals"]}],["left","LV_USR"],["right",{"kind":"reference","refkind":"relative","selectors":["..","..","pers_cat"]}]]}]]}],["1",{"kind":"record","tag":"x-expandable-edit","nodes":[["condition",{"kind":"reference","refkind":"relative","selectors":["$cond"]}],["prefix",{"kind":"reference","refkind":"absolute","selectors":["#0"]}],["target",{"kind":"reference","refkind":"absolute","selectors":["*"]}],["edit",{"kind":"record","tag":"x-edit-listdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[]}],["index","0"]]}]]}],["2",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","$cond"]]}]]}]]}],["index","6"],["pred","5"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Delete rows by value"],["icon","fa-circle-xmark"],["description","Delete rows with value `OTH' in column `pers_cat'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","filter"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","pers_cat"],["cond","equals"],["value","OTH"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","$cond"],["node",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$builtins","equals"]}],["left","OTH"],["right",{"kind":"reference","refkind":"relative","selectors":["..","..","pers_cat"]}]]}]]}],["1",{"kind":"record","tag":"x-expandable-edit","nodes":[["condition",{"kind":"reference","refkind":"relative","selectors":["$cond"]}],["prefix",{"kind":"reference","refkind":"absolute","selectors":["#0"]}],["target",{"kind":"reference","refkind":"absolute","selectors":["*"]}],["edit",{"kind":"record","tag":"x-edit-listdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[]}],["index","0"]]}]]}],["2",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","$cond"]]}]]}]]}],["index","7"],["pred","6"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Delete rows by value"],["icon","fa-circle-xmark"],["description","Delete rows with value `PAS' in column `pers_cat'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","filter"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","pers_cat"],["cond","equals"],["value","PAS"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","$cond"],["node",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$builtins","equals"]}],["left","PAS"],["right",{"kind":"reference","refkind":"relative","selectors":["..","..","pers_cat"]}]]}]]}],["1",{"kind":"record","tag":"x-expandable-edit","nodes":[["condition",{"kind":"reference","refkind":"relative","selectors":["$cond"]}],["prefix",{"kind":"reference","refkind":"absolute","selectors":["#0"]}],["target",{"kind":"reference","refkind":"absolute","selectors":["*"]}],["edit",{"kind":"record","tag":"x-edit-listdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[]}],["index","0"]]}]]}],["2",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","$cond"]]}]]}]]}],["index","8"],["pred","7"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Delete rows by value"],["icon","fa-circle-xmark"],["description","Delete rows with value `EMP' in column `pers_cat'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","filter"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","pers_cat"],["cond","equals"],["value","EMP"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","$cond"],["node",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$builtins","equals"]}],["left","EMP"],["right",{"kind":"reference","refkind":"relative","selectors":["..","..","pers_cat"]}]]}]]}],["1",{"kind":"record","tag":"x-expandable-edit","nodes":[["condition",{"kind":"reference","refkind":"relative","selectors":["$cond"]}],["prefix",{"kind":"reference","refkind":"absolute","selectors":["#0"]}],["target",{"kind":"reference","refkind":"absolute","selectors":["*"]}],["edit",{"kind":"record","tag":"x-edit-listdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[]}],["index","0"]]}]]}],["2",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","$cond"]]}]]}]]}],["index","9"],["pred","8"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Keep rows by value"],["icon","fa-circle-check"],["description","Keep only rows with value `TOTAL' in column `pers_cat'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","filter"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","pers_cat"],["cond","nequals"],["value","TOTAL"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["field","$cond"],["node",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$builtins","nequals"]}],["left","TOTAL"],["right",{"kind":"reference","refkind":"relative","selectors":["..","..","pers_cat"]}]]}]]}],["1",{"kind":"record","tag":"x-expandable-edit","nodes":[["condition",{"kind":"reference","refkind":"relative","selectors":["$cond"]}],["prefix",{"kind":"reference","refkind":"absolute","selectors":["#0"]}],["target",{"kind":"reference","refkind":"absolute","selectors":["*"]}],["edit",{"kind":"record","tag":"x-edit-listdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[]}],["index","0"]]}]]}],["2",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","$cond"]]}]]}]]}],["index","10"],["pred","9"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Drop columns"],["icon","fa-rectangle-xmark"],["description","Drop column `pers_cat'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","drop"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","pers_cat"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","pers_cat"]]}]]}]]}],["index","11"],["pred","10"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-6cdc1d54"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Drop columns"],["icon","fa-rectangle-xmark"],["description","Drop column `c_regis'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","drop"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["column","c_regis"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-recdelete","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["refs","update"],["field","c_regis"]]}]]}]]}],["index","5"],["pred","4"]]},{"kind":"record","tag":"x-edit-append","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-6cdc1d54"],["1","edits"]]}],["node",{"kind":"record","tag":"transform","nodes":[["title","Rename columns"],["icon","fa-i-cursor"],["description","Rename column `geo\\TIME_PERIOD' to `geo'"],["code",{"kind":"record","tag":"x-formula","nodes":[["op",{"kind":"reference","refkind":"absolute","selectors":["$datnicek","table","rename"]}],["inst",{"kind":"record","tag":"x-hole","nodes":[]}],["old","geo\\TIME_PERIOD"],["new","geo"]]}],["edits",{"kind":"list","tag":"edits","nodes":[["0",{"kind":"record","tag":"x-edit-renamefld","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","*"]]}],["old","geo\\TIME_PERIOD"],["new","geo"],["refs","update"]]}]]}]]}],["index","6"],["pred","5"]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-6cdc1d54"]]}],["field","name"],["node","aviaFilter"]]},{"kind":"record","tag":"x-edit-add","nodes":[["target",{"kind":"list","tag":"x-selectors","nodes":[["0","cell-4b11c72b"]]}],["field","name"],["node","railFilter"]]}]"""
