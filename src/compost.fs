...
module Charts = 
  ...
  let render (kind:string) (headers:string list) (data:(Map<string, Primitive>) list) = 
    let key row = CA (match Map.find headers.[0] row with String s -> s | Number n -> string n)
    let value row = CO (match Map.find headers.[1] row with String s -> float s | Number n -> n)
    let viz = 
      Shape.Layered [
        for row in data do
    ...
    h?div [ "class" => "compost" ] [ Compost.createSvg (width - 150.0, 400) viz ]
