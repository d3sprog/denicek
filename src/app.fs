...
type DocumentState = 
  ...
  member x.CurrentDocument = 
    x.Edits.[0 .. x.EditIndex] |> applyHistory x.Initial
  
  member x.FinalDocument = 
    x.Edits |> applyHistory x.Initial
  ...
  let renderHistory trigger histState triggerDoc (docState:DocumentState) = 
    if not histState.Display then [] else [
      h?div [ "id" => "edits" ] [
        ...
        yield h?ol [] [              
          for ieds in List.revNested (List.indexedNested docState.Edits.Groups) do
            for ied in ieds do
              yield renderEdit trigger histState triggerDoc docState ied
            yield h?li [] [ h?hr [] [] ]
        ]
      ] 
    ]
...
let render demos trigger (state:GlobalState) = 
  h?div [] [
    h?header [] [ 
...
let trigger, _ = createVirtualDomApp "out" state (render demos) update
